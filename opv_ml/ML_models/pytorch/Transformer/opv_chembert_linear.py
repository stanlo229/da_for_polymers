...
from opv_ml.ML_models.data.data import OPVDataModule
...
CHECKPOINT_DIR = pkg_resources.resource_filename(
    "opv_ml", "model_checkpoints/Transformer"
)
...
class TransformerModel(pl.LightningModule):
    ...
    def __init__(self, pt_model, n_hidden, output_size, drop_prob, learning_rate):
        ...
        self.linear = nn.Linear(embedding_dim, n_hidden)
    ...
    def forward(self, x):
        ...
        embedded = self.model(x).hidden_states  # [44 x 260 x 768]
        ...
        # print(embedded)
        output = self.linear(embedded)  # [44 x n_hidden]
        output = self.dropout(output)
        output = self.linear2(output)[:, 0]  # [44 x 1]
        # print(output)
