...
from opv_ml.ML_models.pytorch.data.Catalysis_Hein.tokenizer import Tokenizer
import torch
...
TROUBLESHOOT = pkg_resources.resource_filename("opv_ml", "ML_models/pytorch/Transformer/")

SEED_VAL = 4

os.environ["TOKENIZERS_PARALLELISM"] = "false"

# PREDICTION_DIR = pkg_resources.resource_filename("opv_ml")
...
class OPVDataset(Dataset):
    ...
    def get_splits_cv(self, kth_fold, k_fold=10):
        # k_fold=10 --> train = 90%, test = 10%, but we will take validation set from training set
        # --> train = 80%, val = 10% (11.11% of 90%), test = 10%
    ...
    def get_splits_aug_cv(self, aug_ligand_list, yield_array, kth_fold):
        """Function that gets split of train/test/val and then adds augmented training set to train"""
        # splits original dataset into train,val,test
        ...
        total_aug_y = []
        for idx in train.indices:
            aug_da_list = aug_ligand_list[
                idx
            ]  # list of augmented data for each da pair
            i = 0
            j = 0
    ...
    def prepare_data(self):
        ...
        if self.pt_model != None:
            ...
            # convert str to list for DA_pairs
                ...
                yield_array = self.data_aug_smi["E-PR_AY"].to_numpy().astype("float32")
            ...
            # add extra features (Temp, Pd_mol%, Ligand_mol%)
            for index, row in self.data.iterrows():
                ligand_list[index].append(0, self.data["Rxn_temp"][index])
                ligand_list[index].append(1, self.data["Pd_mol%"][index])
                ligand_list[index].append(2, self.data["Ligand_mol%"][index])
            ...
            if self.aug_smiles == 1:
                if self.cv != None:
                    (
                        ...
                        self.yield_val,
                        self.yield_test,
                    ) = yield_dataset.get_splits_aug_cv(
                        aug_list, yield_array, kth_fold=self.cv
                    )
                else:
                    (
                        self.yield_train,
                        ...
                        aug_list, yield_array, seed_val=self.seed_val
                    )
            else:
                if self.cv != None:
                    (
                        ...
                        self.yield_val,
                        self.yield_test,
                    ) = yield_dataset.get_splits_cv(kth_fold=self.cv)
                else:
                    (
                        self.yield_train,
                        self.yield_val,
