...
BRICS_FRAG_DATA = pkg_resources.resource_filename(
    "opv_ml", "data/postprocess/OPV_Troisi/BRICS/master_brics_frag.csv"
)

ERROR_DF = pkg_resources.resource_filename(
    "opv_ml", "data/postprocess/OPV_Troisi/BRICS/bric_errors.csv"
)
...
class BRIC_FRAGS:
    ...
    def bric_frag(self):
        ...
        """
        brics_df = self.data
        brics_df.drop(columns=["SMILES-DFP", "SMILES-AFP"])
        print(brics_df)
        ...
        error_df = pd.DataFrame(
            columns=["index", "DOI", "Donor_SMILES", "Acceptor_SMILES"]
        )
        error_idx = 0
        ...
        for index, row in brics_df.iterrows():
            ...
            # print("DONOR: ", donor_smi, "ACCEPTOR: ", acceptor_smi)
            try:
                donor_brics = list(BRICS.BRICSDecompose(donor_mol, returnMols=True))
            except:
                print("Donor_SMILES went wrong")
                error_df.at[error_idx, "index"] = brics_df.at[index, "index"]
                error_df.at[error_idx, "DOI"] = brics_df.at[index, "DOI"]
                error_df.at[error_idx, "Donor_SMILES"] = brics_df.at[
                    index, "Donor_SMILES"
                ]
                error_idx += 1
            else:
            ...
            try:
                acceptor_brics = list(
                    BRICS.BRICSDecompose(acceptor_mol, returnMols=True)
                )
            except:
                print("Acceptor_SMILES went wrong")
                error_df.at[error_idx, "index"] = brics_df.at[index, "index"]
                error_df.at[error_idx, "DOI"] = brics_df.at[index, "DOI"]
                error_df.at[error_idx, "Acceptor_SMILES"] = brics_df.at[
                    index, "Acceptor_SMILES"
                ]
                error_idx += 1
            else:
        ...
        error_df.to_csv(ERROR_DF, index=False)
